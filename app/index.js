...
// A versão ::text faz o Postgres já devolver o JSON como texto pronto para repassar ao cliente.
// ==================================================================================================
const STMT_GET_EXTRATO = {
  name: 'get-extrato-text',                           // nome estável do statement no servidor
  text: 'SELECT get_extrato($1)::text AS extrato_json',
};
const STMT_PROCESS_TX = {
  name: 'process-transaction-text',                   // outro statement nomeado
  text: 'SELECT process_transaction($1, $2, $3, $4)::text AS response_json',
};
const qGetExtrato = (id) => ({ ...STMT_GET_EXTRATO, values: [id] });                // bind do parâmetro
const qProcessTx = (id, v, t, d) => ({ ...STMT_PROCESS_TX, values: [id, v, t, d] }); // bind dos 4 parâmetros
